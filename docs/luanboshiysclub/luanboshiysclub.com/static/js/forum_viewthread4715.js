...
function setanswer(pid, from){
	if(confirm('您确认要把该回复选为“最佳答案”吗？')){
		...
		$('modactions').action='forum9c9c-2.html?mod=misc&amp;action=bestanswer&amp;tid=' + tid + '&pid=' + pid + '&from=' + from + '&bestanswersubmit=yes';
		$('modactions').submit();
	}
}
...
function fastpostappendreply() {
	if($('fastpostrefresh') != null) {
		...
		if($('fastpostrefresh').checked) {
			location.href = 'forumc168-2.html?mod=redirect&amp;tid='+tid+'&goto=lastpost&random=' + Math.random() + '#lastpost';
			return;
		}
	}
...
function succeedhandle_fastpost(locationhref, message, param) {
	...
	var from = param['from'];
	if(pid) {
		ajaxget('forum2436.html?mod=viewthread&amp;tid=' + tid + '&viewpid=' + pid + '&from=' + from, 'post_new', 'ajaxwaitid', '', null, 'fastpostappendreply()');
		if(replyreload) {
			var reloadpids = replyreload.split(',');
			for(i = 1;i < reloadpids.length;i++) {
				ajaxget('forum2436.html?mod=viewthread&amp;tid=' + tid + '&viewpid=' + reloadpids[i] + '&from=' + from, 'post_' + reloadpids[i]);
			}
		}
...
function succeedhandle_comment(locationhref, message, param) {
	ajaxget('forum905b-2.html?mod=misc&amp;action=commentmore&amp;tid=' + param['tid'] + '&pid=' + param['pid'], 'comment_' + param['pid']);
	hideWindow('comment');
	showCreditPrompt();
}

function succeedhandle_postappend(locationhref, message, param) {
	ajaxget('forum2436.html?mod=viewthread&amp;tid=' + param['tid'] + '&viewpid=' + param['pid'], 'post_' + param['pid']);
...
function replyNotice() {
	var newurl = 'forum0a4c.html?mod=misc&amp;action=replynotice&amp;tid=' + tid + '&op=';
