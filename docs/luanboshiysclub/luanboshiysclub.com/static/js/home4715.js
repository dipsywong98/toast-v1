...
function addSort(obj) {
	if (obj.value == 'addoption') {
		showWindow('addoption', 'home5c5f.html?mod=spacecp&amp;ac=blog&amp;op=addoption&amp;handlekey=addoption&amp;oid='+obj.id);
 	}
}
...
function showreward() {
	...
	var x = new Ajax();
	x.get('home3375.html?mod=misc&amp;ac=ajax&amp;op=getreward', function(s){
		if(s) {
			msgwin(s, 2000);
		}
	});
...
function showBlock(cid, oid) {
	if(parseInt(cid)) {
		var listObj = $(oid);
		var x = new Ajax();
		x.get('portal1d62.html?mod=cp&amp;ac=block&amp;operation=getblock&amp;classid='+cid, function(s){
			listObj.innerHTML = s;
		})
	}
}
...
function friend_changegroup(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('home24c3.html?mod=misc&amp;ac=ajax&amp;op=getfriendgroup&amp;uid='+uid, function(s){
			obj.innerHTML = s;
		});
	}
}
...
function post_add(pid, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('home326a.html?mod=misc&amp;ac=ajax&amp;op=post', function(s){
			newli.innerHTML = s;
		});
...
function post_edit(id, result) {
	if(result) {
		...
		var x = new Ajax();
		x.get('homec96e.html?mod=misc&amp;ac=ajax&amp;op=post&amp;pid='+ pid, function(s){
			obj.innerHTML = s;
		});
	}
}
...
function docomment_get(doid, key) {
	...
	ajaxget('home6bb1.html?mod=spacecp&amp;ac=doing&amp;op=getcomment&amp;handlekey=msg_'+doid+'&doid='+doid+'&key='+key, showid);
	if($(opid)) {
		$(opid).innerHTML = '收起';
...
function docomment_form(doid, id, key) {
	...
	var url = 'home8599.html?mod=spacecp&amp;ac=doing&amp;op=docomment&amp;handlekey=msg_'+id+'&doid='+doid+'&id='+id+'&key='+key;
...
function feedcomment_get(feedid) {
	...
	$(showid).style.display = '';
	ajaxget('homeaae9.html?mod=spacecp&amp;ac=feed&amp;op=getcomment&amp;feedid='+feedid+'&handlekey=feedhk_'+feedid, showid);
	if($(opid) != null) {
		$(opid).innerHTML = '收起';
		$(opid).onclick = function() {
...
function show_click(idtype, id, clickid) {
	ajaxget('home86c3.html?mod=spacecp&amp;ac=click&amp;op=show&amp;clickid='+clickid+'&idtype='+idtype+'&id='+id, 'click_div');
	showCreditPrompt();
}
...
function succeedhandle_pmsend(locationhref, message, param) {
	ajaxget('homec356.html?mod=spacecp&amp;ac=pm&amp;op=viewpmid&amp;pmid=' + param['pmid'], 'pm_append', 'ajaxwaitid', '', null, 'pmsendappend()');
}
...
function setpmstatus(form) {
	...
	if(requesturl) {
		ajaxget('homeea12.html?mod=spacecp&amp;ac=pm&amp;op=setpmstatus' + requesturl, '', 'ajaxwaitid', '', 'none', 'markreadpm(\''+ markreadids.join(',') +'\')');
	}
}
...
function searchpostbyusername(keyword, srchuname) {
	window.location.href = 'search9f31.html?mod=forum&amp;srchtxt=' + keyword + '&srchuname=' + srchuname + '&searchsubmit=yes';
}