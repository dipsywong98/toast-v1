...
function block_get_setting(classname, script, bid) {
	var x = new Ajax();
	x.get('portalc955.html?mod=portalcp&amp;ac=block&amp;op=setting&amp;bid='+bid+'&classname='+classname+'&script='+script+'&inajax=1', function(s){
...
function block_show_thumbsetting(classname, styleid, bid) {
	var x = new Ajax();
	x.get('portald146.html?mod=portalcp&amp;ac=block&amp;op=thumbsetting&amp;classname='+classname+'&styleid='+styleid+'&bid='+bid+'&inajax=1', function(s){
...
function block_pushitem(bid, itemid) {
	...
	if(id && idtype) {
		var x = new Ajax();
		x.get('portal032c.html?mod=portalcp&amp;ac=block&amp;op=push&amp;&amp;bid='+bid+'&itemid='+itemid+'&idtype='+idtype+'&id='+id+'&inajax=1', function(s){
...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	...
	if(confirm(msg)) {
		var url = 'portalf659.html?mod=portalcp&amp;ac=block&amp;op=remove&amp;bid='+bid+'&itemid='+itemid;
		if(itemfrom=='ajax') {
			var x = new Ajax();
			x.get(url+'&inajax=1', function(){
				if(succeedhandle_showblock) succeedhandle_showblock('', '', {'bid':bid});
				showWindow('showblock', 'portal7f3d.html?mod=portalcp&amp;ac=block&amp;op=data&amp;bid='+bid+'&from='+from+'&tab=data&t='+(+ new Date()), 'get', 0);
			});
		} else {
			location.href = url;
		}
...
function portal_comment_requote(cid, aid) {
	var x = new Ajax();
	x.get('portal6ce2.html?mod=portalcp&amp;ac=comment&amp;op=requote&amp;cid='+cid+'&aid='+aid+'&inajax=1', function(s){
		$('message').focus();
...
function searchblock(from) {
	...
	var url = 'portalfeb2.html?mod=portalcp&amp;ac=portalblock&amp;searchkey='+value+'&from='+from;
...
function listblock_bypage(id, idtype) {
	...
	ajaxget('portal91a4.html?mod=portalcp&amp;ac=portalblock&amp;op=recommend&amp;getdata=yes&amp;searchkey='+searchkey+'&targettplname='+tpl+'&id='+id+'&idtype='+idtype, 'itemeditarea');
}
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(bid) {
			...
			ajaxget('portal5860.html?mod=portalcp&amp;ac=block&amp;op=recommend&amp;bid='+bid+'&id='+id+'&idtype='+idtype+'&handlekey=recommenditem', 'itemeditarea');
		} else {
...
function delete_recommenditem(dataid, bid) {
	if(dataid && bid) {
		var x = new Ajax();
		x.get('portal54fa.html?mod=portalcp&amp;ac=block&amp;op=delrecommend&amp;bid='+bid+'&dataid='+dataid+'&inajax=1', function(s){
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('您确定要转换模块的类型从 '+ele.options[0].innerHTML+' 到 '+ele.options[ele.selectedIndex].innerHTML)) {
			ajaxget('portalfffb.html?mod=portalcp&amp;ac=block&amp;op=convert&amp;bid='+bid+'&toblockclass='+ele.value,'blockshow');
		} else {
			ele.selectedIndex = 0;
		}
	}
...
function blockFavorite(bid){
	if(bid) {
		ajaxget('portal092b.html?mod=portalcp&amp;ac=block&amp;op=favorite&amp;bid='+bid,'bfav_'+bid);
	}
}
